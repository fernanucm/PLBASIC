 ...
 *    with RelationName atom. LambdaCut is the current lambda cut which 
 *    allows for filtering out those equations with degrees below this 
 *    threshold.
                ...
                for(j = 0; j < nSize; j++) {
                    ...
                    switch(nTNormId) {
                        ...
                        case TNM_LUKASIEWICZ:
                            ...
                            break;
                        case TNM_DRASTIC:
                            dTNormResult = DRASTIC(array_get_double(pmAdjMatrix, i, k),
                                                   array_get_double(pmAdjMatrix, k, j));
                            break;
                        case TNM_NILPOTENT:
                            dTNormResult = NILPOTENT(array_get_double(pmAdjMatrix, i, k),
                                                     array_get_double(pmAdjMatrix, k, j));
                            break;
                        case TNM_HAMACHER:
                            dTNormResult = HAMACHER(array_get_double(pmAdjMatrix, i, k),
                                                    array_get_double(pmAdjMatrix, k, j));
                            break;
                    }
...
// 	dThreshold = !strcmp(PL_atom_chars(aPrefix),"sim") ? dLambdaCut : 0.0;
  ...
  dThreshold = dLambdaCut;
	...
	/* If a fuzzy relation must be reflexive, adds the generic equation
	   "rel(X, X, 1)" (where X is a free variable) instead of adding a
	   different equation for each symbol.
	   This does not apply to sim.                              */
	if((bReflexive && strcmp(PL_atom_chars(aPrefix),"sim"))) {
